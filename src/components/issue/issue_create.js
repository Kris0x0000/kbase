...
class IssueCreate extends Component {
  ...
  componentDidMount() {
    ...
    if (this.props.location.state) {
      this.setState({
        prev_path: this.props.location.state.prev_path,
  ...
  submit(option) {
    ...
    if (option === "accept") {
      if (
        ...
        this.setState({
          show_warning: true,
          warning_body: "Uzupełnij wszystkie pola przed dodaniem wpisu.",
        });
        setTimeout(() => {
          this.setState({
            show_warning: false,
  ...
  handleAutocompleteChange(event, value) {
    if (value.length > 8) {
      ...
      this.setState({
        to_many_tags: true,
        show_warning: true,
        warning_body: "Nie możesz ustawić więcej niż 8 tagów",
      });
      setTimeout(() => {
        this.setState({
          show_warning: false,
