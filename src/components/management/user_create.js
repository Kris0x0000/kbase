  ...
  componentDidMount() {
    ...
    if (this.props.match.params.id) {
      this.setState((state, props) => {
        return { editmode: true, id: this.props.match.params.id };
      });

      //this.setState({is_loading_set: true});
      axios
        .post(
          getConf("api_url_base") + "/api/user/getUserById",
  ...
  handletextfield(id, data) {
    if (id === "uname") {
      if (!(data === "")) {
        axios
          .post(
            getConf("api_url_base") + "/api/user/getUserByName",
            { username: data },
            { withCredentials: true }
          )
          .then((res) => {
            if (res.data !== "" && res.data._id !== this.state.id) {
              this.setState({
                uname_helper:
                  "Podana nazwa użytkownika istnieje już w systemie.",
                error_uname: true,
              });
      ...
      return (
        <div>
          ...
          <Switch
            id="is_admin"
            onChange={(r) =>
              this.handleswitch(r.target.id, !this.state.is_edited_user_admin)
            }
    ...
    setTimeout(() => {
      if (option === "accept") {
        if (
          ...
          this.state.error_password1 ||
          this.state.error_uname ||
          ...
          if (this.state.editmode && this.state.is_admin) {
            ...
            axios
              .post(
                getConf("api_url_base") + "/api/user/edit",
                {
                  username: this.state.uname,
                  password: this.state.password,
                  is_admin: this.state.is_edited_user_admin,
                  id: this.state.id,
                },
                { withCredentials: true }
              )
            ...
            axios
              .post(
                getConf("api_url_base") + "/api/user/editMyUser",
                { username: this.state.uname, password: this.state.password },
                { withCredentials: true }
              )
              .then((res) => {
            ...
            axios
              .post(
                getConf("api_url_base") + "/api/user/create",
                {
                  username: this.state.uname,
  ...
  render() {
    return (
      <Fragment>
        ...
        >
          <SnackbarContent
            message={this.state.warning_body}
        ...
        <div id="container">
          ...
          <TextField
            ...
            error={this.state.error_password1}
            helperText={this.state.helper_password1}
            id="password"
            label="hasło"
            type="password"
            variant="outlined"
          ...
          <TextField
            ...
            fullWidth={true}
            autoComplete="new-password"
            error={this.state.error_password2}
            helperText={this.state.helper_password2}
            id="password2"
            label="powtórz hasło"
            type="password"
            variant="outlined"
