...
class Management extends Component {
  ...
  handleDelete = (itemid, username) => {
    this.setState({
      ...
      username: username,
      show_warning: true,
      warning_title: "Czy napewno chcesz usunąć tego użytkownika?",
      warning_body: "Ta operacja jest nieodwracalna.",
    });
  };
    ...
    if (item.username !== uname || !item.is_admin) {
      return (
        <Tooltip title="Usuń">
          <IconButton
            color="secondary"
            onClick={() => this.handleDelete(item._id, item.username)}
          >
            <DeleteForeverIcon />
          </IconButton>
        </Tooltip>
  ...
  deleteItem(id, username) {
    ...
    if (username !== uname) {
      axios
        .post(
          getConf("api_url_base") + "/api/user/delete",
          { id: id },
          { withCredentials: true }
        )
        .then((res) => {
        ...
        <div className="bottom_navi">
          ...
          >
            ...
            <Tooltip title="Dodaj użytkownika">
              <IconButton
                color="primary"
                onClick={() => this.setRedirection("", "create")}
              >
                <AddCircleIcon />
              </IconButton>
            </Tooltip>
